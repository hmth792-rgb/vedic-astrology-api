"""
D30 Chart API Routes (Trimsamsha)
"""
import json
import os
...
d30_routes = Blueprint('d30', __name__, url_prefix='/api')
...
@d30_routes.route('/v1/d30-chart-refined', methods=['POST'])
def calculate_d30_chart_refined():
    """
    Calculate D30 (Trimsamsha) chart - Misfortune & Suffering
    ...
    try:
        ...
        d30_longitude_offset = json_data.pop('d30_longitude_offset', None)
        d30_lagna_offset = json_data.pop('d30_lagna_offset', None)
        ...
        default_d30_longitude_offset = float(os.getenv('D30_LONGITUDE_OFFSET', '-0.2025'))
        default_d30_lagna_offset = float(os.getenv('D30_LAGNA_OFFSET', '-0.6667'))
        
        if sidereal_mode is None:
            sidereal_mode = os.getenv('SIDEREAL_MODE', None)

        d30_calculator = D30ChartCalculator(
            ephe_path=ephe_path,
            node_rulership_strategy=node_rulership,
            nakshatra_epsilon=nakshatra_eps,
            sidereal_mode=sidereal_mode,
            ayanamsa_offset=ayanamsa_offset,
            d30_longitude_offset=float(d30_longitude_offset) if d30_longitude_offset is not None else default_d30_longitude_offset,
            d30_lagna_offset=float(d30_lagna_offset) if d30_lagna_offset is not None else default_d30_lagna_offset
        )

        d30_data = d30_calculator.calculate_d30_chart(user_details)
        response = _format_refined_response(d30_data, ephe_path)
