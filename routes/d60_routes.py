"""
D60 Chart API Routes (Shashtiamsha)
"""
import json
import os
...
@d60_routes.route('/v1/d60-chart-refined', methods=['POST'])
def calculate_d60_chart_refined():
    try:
        ...
        d60_longitude_offset = json_data.pop('d60_longitude_offset', None)
        d60_lagna_offset = json_data.pop('d60_lagna_offset', None)
        ...
        d60_calculator = D60ChartCalculator(
            ephe_path=ephe_path,
            node_rulership_strategy=node_rulership,
            nakshatra_epsilon=nakshatra_eps,
            sidereal_mode=sidereal_mode,
            ayanamsa_offset=ayanamsa_offset,
            d60_longitude_offset=float(d60_longitude_offset) if d60_longitude_offset is not None else default_d60_longitude_offset,
            d60_lagna_offset=float(d60_lagna_offset) if d60_lagna_offset is not None else default_d60_lagna_offset
        )
        d60_data = d60_calculator.calculate_d60_chart(user_details)
        response = _format_refined_response(d60_data, ephe_path)
