"""
D16 Chart (Shodash Amsha) Routes
...
# Create blueprint
d16_bp = Blueprint('d16', __name__, url_prefix='/api/v1')

# Initialize schema
user_schema = UserDetailsSchema()


@d16_bp.route('/d16-chart-refined', methods=['POST'])
def calculate_d16_chart_refined():
    """
    Calculate D16 (Shodash Amsha) chart - simplified response with essential graha data only
    ...
    try:
        ...
        d16_calculator = D16ChartCalculator(
            ephe_path=ephe_path,
        ...
        # Calculate D16 chart
        d16_data = d16_calculator.calculate_d16_chart(user_details)

        response = _format_refined_d16_response(d16_data, ephe_path)
...
def _format_refined_d16_response(d16_data: dict, ephe_path: str) -> dict:
    """
    Format D16 data into refined response matching D9 format with named dict structure
    """
    helper = VedicAstrologyHelper()
    ephe_service = SwissEphemerisService(ephe_path=ephe_path)
    planets_data = d16_data['planets']
    ...
    # Add D16 Lagna (Ascendant)
    d16_lagna = d16_data['lagna']
    lagna_nak_lord = d16_lagna.nakshatra_lord  # Use enriched value from calculator
    lagna_sub_lord = d16_lagna.sub_lord  # Use enriched value from calculator
    ...
    lagna_ruler_of = ", ".join([str(h) for h in d16_lagna.ruler_of_houses]) if d16_lagna.ruler_of_houses else "-"
    
    lagna_dict = {
        "Graha": "Lagna",
        "Longitude": format_longitude_dms(d16_lagna.longitude, d16_lagna.sign),
        "Nakshatra": f"{d16_lagna.nakshatra.name.replace('_', ' ').title()} {d16_lagna.nakshatra_pada}",
        "Lord/Sub Lord": lagna_lord_field,
        "Ruler of": lagna_ruler_of,
        "Is In": 1,
        "B. Owner": helper.get_sanskrit_planet_name(d16_data['houses'][0].ruler_planet),
        "Relationship": "-",
        "Dignities": "-"
    }
    ...
    data_dict = {
        ...
        "ayanamsa": round(d16_data['ayanamsa'], 6)
    }
    
    return {
        "status": "success",
        "chart_type": "D16 (Shodash Amsha) - Divisional Chart for Vehicles & Conveyances",
        "data": data_dict
    }
