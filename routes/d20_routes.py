"""
D20 Chart API Routes (Vimshamsha)
"""
import json
import os
...
d20_routes = Blueprint('d20', __name__, url_prefix='/api')
...
@d20_routes.route('/v1/d20-chart-refined', methods=['POST'])
def calculate_d20_chart_refined():
    """
    Calculate D20 (Vimshamsha) chart - Spiritual Progress
    ...
    try:
        ...
        d20_longitude_offset = json_data.pop('d20_longitude_offset', None)
        d20_lagna_offset = json_data.pop('d20_lagna_offset', None)
        ...
        default_d20_longitude_offset = float(os.getenv('D20_LONGITUDE_OFFSET', '-0.1347'))
        default_d20_lagna_offset = float(os.getenv('D20_LAGNA_OFFSET', '9.3907'))
        
        if sidereal_mode is None:
            sidereal_mode = os.getenv('SIDEREAL_MODE', None)

        d20_calculator = D20ChartCalculator(
            ephe_path=ephe_path,
            node_rulership_strategy=node_rulership,
            nakshatra_epsilon=nakshatra_eps,
            sidereal_mode=sidereal_mode,
            ayanamsa_offset=ayanamsa_offset,
            d20_longitude_offset=float(d20_longitude_offset) if d20_longitude_offset is not None else default_d20_longitude_offset,
            d20_lagna_offset=float(d20_lagna_offset) if d20_lagna_offset is not None else default_d20_lagna_offset
        )

        d20_data = d20_calculator.calculate_d20_chart(user_details)
        response = _format_refined_response(d20_data, ephe_path)
