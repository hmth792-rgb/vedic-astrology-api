"""
D45 Chart API Routes (Akshavedamsha)
"""
import json
import os
...
@d45_routes.route('/v1/d45-chart-refined', methods=['POST'])
def calculate_d45_chart_refined():
    try:
        ...
        d45_longitude_offset = json_data.pop('d45_longitude_offset', None)
        d45_lagna_offset = json_data.pop('d45_lagna_offset', None)
        ...
        default_d45_longitude_offset = float(os.getenv('D45_LONGITUDE_OFFSET', '-0.309'))
        default_d45_lagna_offset = float(os.getenv('D45_LAGNA_OFFSET', '0.194'))
        if sidereal_mode is None:
            sidereal_mode = os.getenv('SIDEREAL_MODE', None)

        d45_calculator = D45ChartCalculator(
            ephe_path=ephe_path,
            node_rulership_strategy=node_rulership,
            nakshatra_epsilon=nakshatra_eps,
            sidereal_mode=sidereal_mode,
            ayanamsa_offset=ayanamsa_offset,
            d45_longitude_offset=float(d45_longitude_offset) if d45_longitude_offset is not None else default_d45_longitude_offset,
            d45_lagna_offset=float(d45_lagna_offset) if d45_lagna_offset is not None else default_d45_lagna_offset
        )
        d45_data = d45_calculator.calculate_d45_chart(user_details)
        response = _format_refined_response(d45_data, ephe_path)
