"""
D11 Chart (Ekadash Amsha) Routes
...
from calculators.d11_chart_calculator import D11ChartCalculator
from utils.vedic_helper import VedicAstrologyHelper
from services.swiss_ephemeris_service import SwissEphemerisService

# Create blueprint
d11_bp = Blueprint('d11', __name__, url_prefix='/api/v1')

# Initialize schema
user_schema = UserDetailsSchema()


@d11_bp.route('/d11-chart-refined', methods=['POST'])
def calculate_d11_chart_refined():
    """
    Calculate D11 (Ekadash Amsha) chart - simplified response with essential graha data only
    ...
    try:
        ...
        d11_calculator = D11ChartCalculator(
            ephe_path=ephe_path,
        ...
        # Calculate D11 chart
        d11_data = d11_calculator.calculate_d11_chart(user_details)

        response = _format_refined_d11_response(d11_data, ephe_path)
...
def _format_refined_d11_response(d11_data: dict, ephe_path: str) -> dict:
    """
    Format D11 data into refined response matching D9 format with named dict structure
    """
    helper = VedicAstrologyHelper()
    ephe_service = SwissEphemerisService(ephe_path=ephe_path)
    planets_data = d11_data['planets']
    ...
    # Add D11 Lagna (Ascendant)
    d11_lagna = d11_data['lagna']
    lagna_nak_lord = d11_lagna.nakshatra_lord  # Use enriched value from calculator
    lagna_sub_lord = d11_lagna.sub_lord  # Use enriched value from calculator
    ...
    lagna_ruler_of = ", ".join([str(h) for h in d11_lagna.ruler_of_houses]) if d11_lagna.ruler_of_houses else "-"
    
    lagna_dict = {
        "Graha": "Lagna",
        "Longitude": format_longitude_dms(d11_lagna.longitude, d11_lagna.sign),
        "Nakshatra": f"{d11_lagna.nakshatra.name.replace('_', ' ').title()} {d11_lagna.nakshatra_pada}",
        "Lord/Sub Lord": lagna_lord_field,
        "Ruler of": lagna_ruler_of,
        "Is In": 1,
        "B. Owner": helper.get_sanskrit_planet_name(d11_data['houses'][0].ruler_planet),
        "Relationship": "-",
        "Dignities": "-"
    }
    ...
    data_dict = {
        ...
        "ayanamsa": round(d11_data['ayanamsa'], 6)
    }
    
    return {
        "status": "success",
        "chart_type": "D11 (Ekadash Amsha) - Divisional Chart for Marriage & Wealth",
        "data": data_dict
    }
