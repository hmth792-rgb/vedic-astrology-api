...
# Create blueprint
d10_bp = Blueprint('d10', __name__, url_prefix='/api/v1')

# Initialize schema
user_schema = UserDetailsSchema()


@d10_bp.route('/d10-chart-refined', methods=['POST'])
def calculate_d10_chart_refined():
    """
    Calculate D10 (Dasamsa) chart - refined response
    ...
    try:
        ...
        ayanamsa_offset = float(json_data.pop('ayanamsa_offset', 0.0) or 0.0)
        ...
        d10_calculator = D10ChartCalculator(
            ephe_path=ephe_path,
            node_rulership_strategy=node_rulership,
            nakshatra_epsilon=nakshatra_eps,
            sidereal_mode=sidereal_mode,
        ...
        d10_data = d10_calculator.calculate_d10_chart(user_details, d1_chart)

        response = _format_refined_d10_response(d10_data)
...
def _format_refined_d10_response(d10_data: dict) -> dict:
    """Format D10 data into refined response matching D9 format with named dict structure"""
    ...
    planets_data = d10_data['planets']
    lagna = d10_data['lagna']
    ...
    # Add D10 Lagna
    d10_lagna = lagna
    lagna_nak_entry = next((n for n in ephe_service.nakshatras if n["name"] == d10_lagna.nakshatra), None)
    ...
    lagna_dict = {
        "Graha": "Lagna",
        "Longitude": format_longitude_dms(d10_lagna.longitude, d10_lagna.sign),
        "Nakshatra": f"{d10_lagna.nakshatra.name.replace('_', ' ').title()} {d10_lagna.nakshatra_pada}",
        "Lord/Sub Lord": lagna_lord_field,
        "Ruler of": "-",
        "Is In": 1,
        "B. Owner": helper.get_sanskrit_planet_name(d10_data['houses'][0].ruler_planet),
        "Relationship": "-",
        "Dignities": "-"
    }
    ...
    data_dict = {
        ...
        "ayanamsa": round(d10_data['ayanamsa'], 6)
    }

    return {
        "status": "success",
        "chart_type": "D10 (Dasamsha) - Divisional Chart for Career & Status",
        "data": data_dict
    }
