...
@d27_routes.route('/v1/d27-chart-refined', methods=['POST'])
def calculate_d27_chart_refined():
    """
    Calculate D27 (Bhamsa) chart - refined response
    """
    try:
        ...
        d27_longitude_offset = json_data.pop('d27_longitude_offset', None)
        d27_lagna_offset = json_data.pop('d27_lagna_offset', None)
        ...
        default_d27_lagna_offset = float(os.getenv('D27_LAGNA_OFFSET', '-0.6'))

        if sidereal_mode is None:
            sidereal_mode = os.getenv('SIDEREAL_MODE', None)

        d27_calculator = D27ChartCalculator(
            ephe_path=ephe_path,
            node_rulership_strategy=node_rulership,
            nakshatra_epsilon=nakshatra_eps,
            sidereal_mode=sidereal_mode,
            ayanamsa_offset=ayanamsa_offset,
            d27_longitude_offset=float(d27_longitude_offset) if d27_longitude_offset is not None else default_d27_longitude_offset,
            d27_lagna_offset=float(d27_lagna_offset) if d27_lagna_offset is not None else default_d27_lagna_offset
        )

        d27_data = d27_calculator.calculate_d27_chart(user_details)
        response = _format_refined_response(d27_data, ephe_path)
