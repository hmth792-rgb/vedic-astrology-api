...
# Create blueprint
d4_bp = Blueprint('d4', __name__, url_prefix='/api/v1')

# Initialize schema
user_schema = UserDetailsSchema()


@d4_bp.route('/d4-chart-refined', methods=['POST'])
def calculate_d4_chart_refined():
    """
    Calculate D4 (Chaturthamsa) chart - refined response
    ...
    try:
        ...
        d4_calculator = D4ChartCalculator(
            ephe_path=ephe_path,
            node_rulership_strategy=node_rulership,
            nakshatra_epsilon=nakshatra_eps,
            sidereal_mode=sidereal_mode
        ...
        d4_data = d4_calculator.calculate_d4_chart(user_details, d1_chart)

        response = _format_refined_d4_response(d4_data)
...
def _format_refined_d4_response(d4_data: dict) -> dict:
    """Format D4 data into refined response matching D9 format with named dict structure"""
    ...
    # Add D4 Lagna
    d4_lagna = lagna
    lagna_nak_entry = next((n for n in ephe_service.nakshatras if n["name"] == d4_lagna.nakshatra), None)
    ...
    lagna_dict = {
        "Graha": "Lagna",
        "Longitude": format_longitude_dms(d4_lagna.longitude, d4_lagna.sign),
        "Nakshatra": f"{d4_lagna.nakshatra.name.replace('_', ' ').title()} {d4_lagna.nakshatra_pada}",
        "Lord/Sub Lord": lagna_lord_field,
        "Ruler of": "-",
        "Is In": 1,
        "B. Owner": helper.get_sanskrit_planet_name(d4_data['houses'][0].ruler_planet),
        "Relationship": "-",
        "Dignities": "-"
    }
    ...
    data_dict = {
        ...
        "ayanamsa": round(d4_data['ayanamsa'], 6)
    }

    return {
        "status": "success",
        "chart_type": "D4 (Chaturthamsa) - Divisional Chart for Property & Assets",
        "data": data_dict
    }
