...
# Create blueprint
d6_bp = Blueprint('d6', __name__, url_prefix='/api/v1')

# Initialize schema
user_schema = UserDetailsSchema()


@d6_bp.route('/d6-chart-refined', methods=['POST'])
def calculate_d6_chart_refined():
    """
    Calculate D6 (Shashtamsha) chart - refined response
    ...
    try:
        ...
        d6_calculator = D6ChartCalculator(
            ephe_path=ephe_path,
            node_rulership_strategy=node_rulership,
            nakshatra_epsilon=nakshatra_eps,
            sidereal_mode=sidereal_mode
        ...
        d6_data = d6_calculator.calculate_d6_chart(user_details, d1_chart)

        response = _format_refined_d6_response(d6_data)
...
def _format_refined_d6_response(d6_data: dict) -> dict:
    """Format D6 data into refined response matching D9 format with named dict structure"""
    ...
    # Add D6 Lagna
    d6_lagna = lagna
    lagna_nak_entry = next((n for n in ephe_service.nakshatras if n["name"] == d6_lagna.nakshatra), None)
    ...
    lagna_dict = {
        "Graha": "Lagna",
        "Longitude": format_longitude_dms(d6_lagna.longitude, d6_lagna.sign),
        "Nakshatra": f"{d6_lagna.nakshatra.name.replace('_', ' ').title()} {d6_lagna.nakshatra_pada}",
        "Lord/Sub Lord": lagna_lord_field,
        "Ruler of": "-",
        "Is In": 1,
        "B. Owner": helper.get_sanskrit_planet_name(d6_data['houses'][0].ruler_planet),
        "Relationship": "-",
        "Dignities": "-"
    }
    ...
    data_dict = {
        ...
        "ayanamsa": round(d6_data['ayanamsa'], 6)
    }

    return {
        "status": "success",
        "chart_type": "D6 (Shashtamsha) - Divisional Chart for Health & Enemies",
        "data": data_dict
    }
