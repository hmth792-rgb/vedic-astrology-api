...
# Create blueprint
d7_bp = Blueprint('d7', __name__, url_prefix='/api/v1')

# Initialize schema
user_schema = UserDetailsSchema()


@d7_bp.route('/d7-chart-refined', methods=['POST'])
def calculate_d7_chart_refined():
    """
    Calculate D7 (Saptamsha) chart - refined response
    ...
    try:
        ...
        d7_calculator = D7ChartCalculator(
            ephe_path=ephe_path,
            node_rulership_strategy=node_rulership,
            nakshatra_epsilon=nakshatra_eps,
            sidereal_mode=sidereal_mode
        ...
        d7_data = d7_calculator.calculate_d7_chart(user_details, d1_chart)

        response = _format_refined_d7_response(d7_data)
...
def _format_refined_d7_response(d7_data: dict) -> dict:
    """Format D7 data into refined response matching D9 format with named dict structure"""
    ...
    # Add D7 Lagna
    d7_lagna = lagna
    lagna_nak_entry = next((n for n in ephe_service.nakshatras if n["name"] == d7_lagna.nakshatra), None)
    ...
    lagna_dict = {
        "Graha": "Lagna",
        "Longitude": format_longitude_dms(d7_lagna.longitude, d7_lagna.sign),
        "Nakshatra": f"{d7_lagna.nakshatra.name.replace('_', ' ').title()} {d7_lagna.nakshatra_pada}",
        "Lord/Sub Lord": lagna_lord_field,
        "Ruler of": "-",
        "Is In": 1,
        "B. Owner": helper.get_sanskrit_planet_name(d7_data['houses'][0].ruler_planet),
        "Relationship": "-",
        "Dignities": "-"
    }
    ...
    data_dict = {
        ...
        "ayanamsa": round(d7_data['ayanamsa'], 6)
    }

    return {
        "status": "success",
        "chart_type": "D7 (Saptamsha) - Divisional Chart for Children & Progeny",
        "data": data_dict
    }
