...
def _format_refined_d8_response(d8_data: dict) -> dict:
    """Format D8 data into refined response matching D9 format with named dict structure"""
    ...
    # Add D8 Lagna
    d8_lagna = lagna
    lagna_nak_entry = next((n for n in ephe_service.nakshatras if n["name"] == d8_lagna.nakshatra), None)
    ...
    lagna_dict = {
        "Graha": "Lagna",
        "Longitude": format_longitude_dms(d8_lagna.longitude, d8_lagna.sign),
        "Nakshatra": f"{d8_lagna.nakshatra.name.replace('_', ' ').title()} {d8_lagna.nakshatra_pada}",
        "Lord/Sub Lord": lagna_lord_field,
        "Ruler of": "-",
        "Is In": 1,
        "B. Owner": helper.get_sanskrit_planet_name(d8_data['houses'][0].ruler_planet),
        "Relationship": "-",
        "Dignities": "-"
    }
    ...
    data_dict = {
        ...
        "ayanamsa": round(d8_data['ayanamsa'], 6)
    }

    return {
        "status": "success",
        "chart_type": "D8 (Ashtamsha) - Divisional Chart for Longevity & Obstacles",
        "data": data_dict
    }
