"""
D40 Chart (Khavedamsha) Calculator
Divisional chart for maternal lineage and auspicious/inauspicious influences
D40 divides each zodiac sign into 40 equal parts (0.75 degrees each)
"""
...
class D40ChartCalculator:
    """Calculator for D40 Khavedamsha chart"""
    ...
    def __init__(self, ephe_path: str = "./ephe", nakshatra_epsilon: float = 1e-6,
        ...
        self.d40_longitude_offset = d40_longitude_offset
        self.d40_lagna_offset = d40_lagna_offset
    ...
    def calculate_d40_chart(self, user_details: UserDetails, d1_chart: D1Chart = None) -> Dict:
        if d1_chart is None:
            d1_chart = self.d1_calculator.calculate_d1_chart(user_details)

        d40_planets = [self._convert_to_d40(planet) for planet in d1_chart.planets]
        d40_lagna = self._convert_to_d40_with_offset(d1_chart.lagna)
        d40_houses = self._calculate_d40_houses(d40_lagna, d40_planets)
        d40_lagna = self._enrich_planet_with_vedic_details(d40_lagna, d40_houses, d40_planets)
        enriched_planets = [self._enrich_planet_with_vedic_details(planet, d40_houses, d40_planets) for planet in d40_planets]

        return {
            "chart_type": "D40 (Khavedamsha)",
            "description": "Divisional chart for maternal lineage and subtle fortune influences",
            "lagna": d40_lagna,
    ...
    def _convert_to_d40(self, planet_position: PlanetPosition) -> PlanetPosition:
        ...
        start_sign_num = self._get_d40_start_sign(d1_sign_num)
        d40_sign_num = ((start_sign_num - 1 + portion - 1) % 12) + 1
        d40_degree = (d1_degree_in_sign * 40.0) % 30.0
        d40_longitude = (d40_sign_num - 1) * 30.0 + d40_degree
        d40_longitude, d40_sign_num, d40_degree = self._apply_d40_corrections(d40_longitude, self.d40_longitude_offset)
        nakshatra, pada = self.ephemeris_service.longitude_to_nakshatra(d40_longitude + self.nakshatra_epsilon)
    ...
    def _convert_to_d40_with_offset(self, lagna_position: PlanetPosition) -> PlanetPosition:
        ...
        start_sign_num = self._get_d40_start_sign(adjusted_sign_num)
        d40_sign_num = ((start_sign_num - 1 + portion - 1) % 12) + 1
        d40_degree = (d1_degree_in_sign * 40.0) % 30.0
        d40_longitude = (d40_sign_num - 1) * 30.0 + d40_degree
        d40_longitude, d40_sign_num, d40_degree = self._apply_d40_corrections(d40_longitude, self.d40_lagna_offset)
        nakshatra, pada = self.ephemeris_service.longitude_to_nakshatra(d40_longitude + self.nakshatra_epsilon)
    ...
    def _get_d40_start_sign(self, sign_num: int) -> int:
        return Zodiac.ARIES.value if sign_num % 2 == 1 else Zodiac.LIBRA.value

    def _apply_d40_corrections(self, longitude: float, offset: float) -> tuple:
    ...
    def _calculate_d40_houses(self, d40_lagna: PlanetPosition, d40_planets: List[PlanetPosition]) -> List[HouseData]:
        houses = []
        lagna_sign = d40_lagna.sign
        for house_num in range(1, 13):
            sign_num = ((lagna_sign.value - 1 + house_num - 1) % 12) + 1
            sign = Zodiac(sign_num)
            cusp_longitude = (sign_num - 1) * 30
            planets_in_house = [p for p in d40_planets if p.sign == sign]
